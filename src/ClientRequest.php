...
class ClientRequest
{
    ...
    public function request(string $method, string $uri, ?array $body = null): ResponseInterface
    {
        try {
            ...
            return $this->client->request($method, "{$apiUri}{$uri}", $options);
        } catch (GuzzleException $e) {
            throw new Exception("Erro ao fazer requisição: " . $e->getMessage(), (int) $e->getCode());
        }
    }

    public function requestRaw(string $method, string $uri, ?array $body = null): ResponseInterface
    {
