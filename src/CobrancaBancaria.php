...
class CobrancaBancaria
{
    ...
    public function listarBoletosPorPagador(CodigoSituacaoParam $codigoSituacao, string $dataInicio, string $dataFim, string $numeroCpfCnpj): array
    {
        try {
            ...
            $params = "?numeroCliente={$this->numeroContrato}&codigoSituacao={$codigoSituacao->value}&dataInicio={$dataInicio}&dataFim={$dataFim}";
    ...
    /**
     * @param FaixaNossoNumero[] $filtros - Máximo 1 item
     * @throws Exception
     * @link https://documenter.getpostman.com/view/20565799/Uzs6yNhe
     */
    public function consultarFaixasNossoNumeroDisponiveis(FaixaNossoNumero $filtro): array
    {
        try {
            $body = $filtro->toArray();
            $clientRequest = new ClientRequest($this->authenticator);

            $result = $clientRequest->request('GET', "/boletos/faixas-nosso-numero-disponiveis?numeroCliente={$body['numeroCliente']}&codigoModalidade={$body['codigoModalidade']}&quantidade={$body['quantidade']}&numeroContratoCobranca={$body['numeroContratoCobranca']}");

            $resultArray = json_decode($result->getBody()->getContents());

            $faixas = [];
            foreach ($resultArray->resultado as $item) {
                $faixas[] = FaixaNossoNumero::createFromJsonArray($item);
            }

            return $faixas;
        } catch (Exception $e) {
            throw new Exception('Ocorreu um erro ao consultar faixas de nosso número: ' . $e->getMessage());
        }
    }
     ...
     * @link https://documenter.getpostman.com/view/20565799/Uzs6yNhe
     */
    public function comandarBaixaBoleto(int $nossoNumero, string $numeroCpfCnpj, int $codigoModalidade = 1): array
    {
        try {
            $clientRequest = new ClientRequest($this->authenticator);

            $body = [
                'numeroCliente' => $this->numeroContrato,
                'codigoModalidade' => $codigoModalidade,
            ];

            $result = $clientRequest->request('POST', "/boletos/{$nossoNumero}/baixar", $body);
            ...
            throw new Exception('Ocorreu um erro ao comandar a baixa do boleto: ' . $e->getMessage());
        }
    }
    ...
    public function consultarSituacaoSolicitacaoMovimentacao(int $codigoSolicitacao): array
    {
        try {
            ...
            $params = "?numeroCliente={$this->numeroContrato}&codigoSolicitacao={$codigoSolicitacao}";
    ...
    public function downloadArquivoMovimentacao(int $codigoSolicitacao, int $idArquivo): array
    {
        try {
            ...
            $params = "?numeroCliente={$this->numeroContrato}&codigoSolicitacao={$codigoSolicitacao}&idArquivo={$idArquivo}";

            $result = $clientRequest->request('GET', "/boletos/movimentacoes/download{$params}");
